Namespace MediaFormats
    ...
    Public Module PCB
        ...
        Public Sub ProcessPCBFile(ByVal sFile As String, Optional ByVal aFile As Byte() = Nothing)
            If bDebug Then : Console.WriteLine("Process PCB File: " & sFile) : End If
                        ...
                        Select Case AnsiMode
                            ...
                            Case 2
                                If UBound(aAnsi) >= iLoop + 1 Then
                                    If ConverterSupport.isHex(Chr(CurChr)) And ConverterSupport.isHex(Chr(aAnsi(iLoop + 1))) Then
                                        BackColor = ConverterSupport.smaller(ConverterSupport.HexToDec(CStr(Chr(CurChr))), 7)
                                        ForeColor = ConverterSupport.HexToDec(CStr(Chr(aAnsi(iLoop + 1))))
                                ...
                                If bDrawChar = True Then
                                    If ConverterSupport.SetChar(Chr(64)) = False Then
                                        iLoop = UBound(aAnsi) + 1
                                    Else
                                        If ConverterSupport.SetChar(Chr(88)) Then
                                            AnsiMode = 0
                                        Else
                                            iLoop = UBound(aAnsi) + 1
                                        End If
