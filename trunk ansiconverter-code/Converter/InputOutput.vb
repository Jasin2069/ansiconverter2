...
Namespace ConverterSupport
    ...
    Public Module InputOutput
        ...
        Public Function checkFileFormat(ByVal sFile As String) As FFormats
            ...
            Dim ofile As System.IO.FileStream
            If IO.File.Exists(sFile) Then
                If GetFileSizeNum(sFile) > 0 Then
            ...
            If sData <> "" And Not sData Is Nothing Then
                ...
                Else
                    ...
                    Or InStr(1, sData, "&nbsp;", CompareMethod.Text) > 0 Or InStr(1, sData, ";&#", CompareMethod.Text) > 0 Then
                        sRes = FTypes.HTML
                    Else
                        If ConverterSupport.RegExTest(sData, "@X[0-9A-F]{2}", RegularExpressions.RegexOptions.Singleline) = True Then
                            sRes = FTypes.PCB
                        Else
                            If ConverterSupport.RegExTest(sData, "@[0-9A-F]{2}@", RegularExpressions.RegexOptions.Singleline) = True Then
                                sRes = FTypes.WC3
                            Else
        ...
        Public Function GetFileSize(ByVal sFile As String) As String
            ...
            If lSize > 1024000000 Then 'GB
                slSize = CStr(ConverterSupport.USStringRound(lSize / 1024000000, 2)) & " GB"
            Else
                If lSize > 1024000 Then 'MB
                    slSize = CStr(ConverterSupport.USStringRound(lSize / 1024000, 2)) & " MB"
                Else
                    If lSize > 1024 Then     'KB
                        slSize = CStr(ConverterSupport.USStringRound(lSize / 1024, 2)) & " KB"
                    Else  'Bytes
                        slSize = CStr(lSize) & " B"
                    End If
                End If
        ...
        Public Function GetFileSizeNum(ByVal sFile As String) As Long
            Dim oFile As FileInfo
            oFile = New FileInfo(sFile)
            Return oFile.Length
            oFile = Nothing
                ...
                For x = minX To maxX
                    ...
                    If iWhatChange <> 0 Then
                        ...
                        If iWhatChange > 9 Then 'all changed
                            ...
                            aOut(Cnt) = 59 : Cnt += 1
                            aOut(Cnt) = Asc(Left(Internal.AnsiForeColors(ForeColor), 1)) : Cnt += 1
                            aOut(Cnt) = Asc(Right(Internal.AnsiForeColors(ForeColor), 1)) : Cnt += 1
                            aOut(Cnt) = 59 : Cnt += 1
                            aOut(Cnt) = Asc(Left(Internal.AnsiBackColors(BackColor), 1)) : Cnt += 1
                            aOut(Cnt) = Asc(Right(Internal.AnsiBackColors(BackColor), 1)) : Cnt += 1
                            aOut(Cnt) = 109 : Cnt += 1
                        Else
